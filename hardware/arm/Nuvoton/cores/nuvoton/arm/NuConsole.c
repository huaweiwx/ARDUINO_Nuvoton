 ...
 *
 * add for arduino stream by huaweiwx@sina.com 2018.11.25
*****************************************************************************/
...
/*add for stream huaweiwx@sina.com 2018.11.25*/
uint32_t NuConsole_available(void){
	return _Get_Data_Size(&NuConsole_InfoBlock.aRxFIFO);
}

int NuConsole_peek(void){
	if(_Get_Data_Size(&NuConsole_InfoBlock.aRxFIFO)) 
		 return g_acRxBuffer[NuConsole_InfoBlock.aRxFIFO.uTail];
	else
         return -1;		
}

uint32_t NuConsole_availableForWrite(void){
	return _Get_Free_Size(&NuConsole_InfoBlock.aTxFIFO);
}