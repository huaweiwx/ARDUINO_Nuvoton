/* forth_spisdcard.ino stm32 vdisk/mcu administer
    ---------------------------------------------------------------------------------------
   from the menu
...
/* ----------------This function implements the "pm" command. Set a pin input/output mode  --------------------------*/
// Usage: script forthstring
static int Cmd_script(int argc, char *argv[])
{
  if (argc > 1) {
    shell.execute((__FlashStringHelper*)argv[1]);
  }
  return (0);
}
// Usage: scriptset name forthstring
static int Cmd_scriptSet(int argc, char *argv[])
{
  if (argc > 1) {
    shell.set((__FlashStringHelper*)argv[1],(__FlashStringHelper*)argv[2]);
  }
...
//forth script  
  { "scriptset", Cmd_scriptSet,  ": forthshell set" } ,
  { "script", Cmd_script,        "   : forthshell exec" } ,
