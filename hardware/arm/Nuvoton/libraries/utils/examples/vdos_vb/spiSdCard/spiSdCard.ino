...
void setup() {
  ...
  Serial << "\n" << HEADSTAR;
  Serial.println("*        STM32 vdos demo v1.0          *");
  Serial.println("*  Type help Display list of commands  *");
  Serial.println(HEADSTAR);
...
#if defined(STM32F1)
      print_port((char*)GPIO_CRH, GPIOA->CRH & 0xffff);
      print_port((char*)GPIO_CRL, GPIOA->CRL & 0xffff);
#else
...
#if defined(STM32F1)
      print_port((char*)GPIO_CRH, GPIOB->CRH & 0xffff);
      print_port((char*)GPIO_CRL, GPIOB->CRL & 0xffff);
#else
...
#if defined(STM32F1)
      print_port((char*)GPIO_CRH, GPIOC->CRH & 0xffff);
      print_port((char*)GPIO_CRL, GPIOC->CRL & 0xffff);
#else
...
#if defined(STM32F1)
      print_port((char*)GPIO_CRH, GPIOD->CRH & 0xffff);
      print_port((char*)GPIO_CRL, GPIOD->CRL & 0xffff);
#else
...
#if defined(STM32F1)
      print_port((char*)GPIO_CRH, GPIOE->CRH & 0xffff);
      print_port((char*)GPIO_CRL, GPIOE->CRL & 0xffff);
#else
...
#if defined(STM32F1)
      print_port((char*)GPIO_CRH, GPIOF->CRH & 0xffff);
      print_port((char*)GPIO_CRL, GPIOF->CRL & 0xffff);
#else
...
#if defined(STM32F1)
      print_port((char*)GPIO_CRH, GPIOG->CRH & 0xffff);
      print_port((char*)GPIO_CRL, GPIOG->CRL & 0xffff);
#else
...
#if defined(STM32F1)
      print_port((char*)GPIO_CRH, GPIOH->CRH & 0xffff);
      print_port((char*)GPIO_CRL, GPIOH->CRL & 0xffff);
#else
...
char vbprog[1024] = MYPROG;
void Executable(vector <CSub> Subs);

//exp: run mydemo.bas
int Cmd_run(int argc, char *argv[]) {
  vector<CSub> m_Subs;
  static uint16_t i = sizeof(vbprog);
  ...
  CMake vbMake(vbprog, i); // 创建Make对象,用来生成class代码
...
{
  { "?",   Cmd_help,      " : List of cmds & get info..."} ,

  //debug
  { "d",      Cmd_dispmen,   "    : disp memory"} ,

  //vdisk function
  { "dir",    Cmd_dir,      "  : list vdisk files"} ,
  { "ls",     Cmd_dir,      "   : alias for dir"} ,
#if defined(STM32F4)||(FLASH_BANK1_END >  0x0801FFFFU)   //for hight density  xC/D/E
  { "load",   Cmd_load,      " : chech binfile & load it if available"} ,
  //vb
  { "run",    Cmd_run,      "  : rub vb"} ,
#endif
  { "type",   Cmd_type,      " : type a txtfile"} ,
  ...
  { "dw",     Cmd_digitalWrite, "   : digitalWrite" } ,
  { "dt",     Cmd_digitalToggle, "   : digitalToggle"  },
  { "aw",     Cmd_analogWrite, "   : analogWrite)"} ,
  { "ar",     Cmd_analogRead,  "   : analogRead"},

  { "si",     Cmd_shiftin,     "   : shiftIn"} ,
  { "so",     Cmd_shiftout,    "   : shiftOut"},
  { "pulse",  Cmd_pulsein,     ": pulseIn"} ,
  { "ee",     Cmd_eeprom,    "   : i2c eeprom"},
